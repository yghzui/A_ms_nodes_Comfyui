...
2. 前端分别导出 `mask_data` 与 `paint_data` 两张透明 PNG 的 Base64；如果本次应用过裁剪，则先把裁剪后的基础图上传成一张新图片。
...
前端只需要提供两份透明 PNG 数据；如果启用了裁剪，保存前还需要先准备一张新的基础图：

- `mask_data`：遮罩层图像。
  涂抹区域的 `Alpha` 应为 `255`，未涂抹区域应为 `0`。
- `paint_data`：彩色笔触图像。
  背景应保持完全透明，只保留彩色笔触本身。
- `cropped_base_image`：不是传给 `/a_my_nodes/upload_custom_edited_image` 的额外字段，而是在保存前先通过 ComfyUI 通用的 `/upload/image` 上传的一张新基础图。
- 这张图一旦上传成功，后续 `mask_data` 与 `paint_data` 就都相对于这张新图工作，不再和裁剪前的原图坐标发生关系。
...
点击“应用裁剪”后，前端会立即把画布切到新的尺寸，并继续在裁剪后的结果上编辑；这一步只影响当前前端会话，不立即改变后端文件。

真正点击保存时，如果当前基础图已经被裁剪过，前端会先把“裁剪后的基础图”通过 `/upload/image` 上传成一张新的图片，再把这张新图片路径作为 `image_path` 传给 `/a_my_nodes/upload_custom_edited_image`。这样后续保存链路仍然复用旧逻辑，不需要给原接口增加裁剪参数。
...
- `/a_my_nodes/upload_custom_edited_image` 本身不接收裁剪参数。
- 如果用户在前端点过“应用裁剪”，前端会先把裁剪后的基础图上传成一张新的 `input` 图片。
- 然后再把这张新图片路径作为 `image_path` 传给旧接口。
- 因此，对旧接口来说，它看到的始终是一张“已经裁剪完成的新图”，后续 `mask` 与 `paint` 仍按原有方式处理。
...
2. 如果这次包含裁剪，应把“保存前先上传得到的那张裁剪后基础图路径”记下来，作为后续继续编辑时的基础图路径。
3. 把最终图路径写回 `LoadImageBatchAdvanced` 节点的图片路径参数。
4. 再将更新后的工作流提交给 `/prompt`。
