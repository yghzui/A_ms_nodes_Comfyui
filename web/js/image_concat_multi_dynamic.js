...
console.log("Loading custom node: A_my_nodes/web/js/image_concat_multi_dynamic.js");

import { app } from "../../../scripts/app.js";

app.registerExtension({
    name: "A_my_nodes.ImageConcatMultiMs.DynamicInputs",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        // 只对 ImageConcatMulti 节点进行操作
        if (nodeData.name === "ImageConcatMultiMs") {
            ...
            nodeType.prototype.onConnectionsChange = function(connectionType, slotIndex, isConnected, linkInfo, inputInfo) {
                ...
                if (inputName && inputName.startsWith("image_")) {
                    if (isConnected) {
                        ...
                        // 检查下一个输入是否已存在
                        const nextInputName = `image_${nextIndex}`;
                        if (!this.inputs.find(input => input.name === nextInputName) && nextIndex <= 20) {
                            // 添加下一个输入
                            this.addInput(nextInputName, "IMAGE");
            ...
            nodeType.prototype.cleanupImageInputs = function() {
                ...
                for (let i = 0; i < imageInputs.length; i++) {
                    const inputIndex = this.inputs.indexOf(imageInputs[i]);
                ...
                for (let i = imageInputs.length - 1; i >= keepCount; i--) {
                    const inputIndex = this.inputs.indexOf(imageInputs[i]);
                    this.removeInput(inputIndex);
                }
                
                // 确保有一个空白输入在最后
                if (imageInputs.length < keepCount) {
