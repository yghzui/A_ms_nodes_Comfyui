...
#
#
# def shift_image(image: Image, distance_x: int, distance_y: int, background_color: str = '#000000',
#                 cyclic: bool = False) -> Image:
#     width = image.width
#     height = image.height
...
#     if min(dx, dy) > 100:
#         np_img = torch.nn.functional.interpolate(images.detach().clone().movedim(-1, 1), scale_factor=0.1,
#                                                  mode='bilinear').movedim(1, -1).cpu().numpy()
#         for index, image in enumerate(np_img):
#             np_img[index] = cv2.GaussianBlur(image, (dx // 20 * 2 + 1, dy // 20 * 2 + 1), 0)
#         return torch.nn.functional.interpolate(torch.from_numpy(np_img).movedim(-1, 1),
#                                                size=(images.shape[1], images.shape[2]), mode='bilinear').movedim(1, -1)
#     else:
...
#
#
# def image_add_grain(image: Image, scale: float = 0.5, strength: float = 0.5, saturation: float = 0.7, toe: float = 0.0,
#                     seed: int = 0) -> Image:
...
#
#
# def radialblur_image(image: Image, blur_strength: float, center_x: float, center_y: float, focus_spread: float,
#                      steps: int = 5) -> Image:
...
#
#
# def depthblur_image(image: Image, depth_map: Image, blur_strength: float, focal_depth: float, focus_spread: float,
#                     steps: int = 5) -> Image:
#     width, height = image.size
#     image = pil2tensor(image)
...
#             img_down_scaled = img_rotated.resize((img_rotated.width // SSAA, img_rotated.height // SSAA),
#                                                  resize_sampler)
...
#
#
# def image_rotate_extend_with_alpha(image: Image, angle: float, alpha: Image = None, method: str = "lanczos",
#                                    SSAA: int = 0) -> tuple:
...
#
#
# def create_box_gradient(start_color_inhex: str, end_color_inhex: str, width: int, height: int,
#                         scale: int = 50) -> Image:
...
#
#
# def create_gradient(start_color_inhex: str, end_color_inhex: str, width: int, height: int,
#                     direction: str = 'bottom') -> Image:
...
#
#
# def generate_text_image(width: int, height: int, text: str, font_file: str, text_scale: float = 1,
#                         font_color: str = "#FFFFFF", ) -> Image:
...
#         [max(target_color[0] - tolerance, 0), max(target_color[1] - tolerance, 0), max(target_color[2] - tolerance, 0)])
#     upper_color = np.array([min(target_color[0] + tolerance, 255), min(target_color[1] + tolerance, 255),
#                             min(target_color[2] + tolerance, 255)])
#
#     # 创建掩码
...
# def mask_edge_detail(image: torch.Tensor, mask: torch.Tensor, detail_range: int = 8, black_point: float = 0.01,
#                      white_point: float = 0.99) -> torch.Tensor:
...
#                       max_megapixels: float = 2.0) -> Image:
#     if image.mode != 'RGB':
#         image = image.convert('RGB')
#     if trimap.mode != 'L':
...
#     model_file_path = ""
#     try:
#         model_file_path = os.path.join(os.path.normpath(folder_paths.folder_names_and_paths[model_folder_name][0][0]),
#                                        model_name)
#     except:
#         pass
...
#
#
# def mask_fix(images: torch.Tensor, radius: int, fill_holes: int, white_threshold: float,
#              extra_clip: float) -> torch.Tensor:
#     d = radius * 2 + 1
