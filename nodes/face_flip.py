...
def detect_face_orientation(image: np.ndarray, detection_threshold: float = 0.5) -> str:
    """
    使用MediaPipe检测人脸朝向
...
class FaceFlip:
    ...
    def flip_images(self, image, auto_flip_face, target_orientation, horizontal_flip, vertical_flip, detection_threshold):
        ...
        for i in range(batch_size):
            ...
            if auto_flip_face:
                # 检测人脸朝向
                face_orientation = detect_face_orientation(img_cv, detection_threshold)
...
if __name__ == "__main__":
    ...
    # 读取测试图片
    image_path = r"D:\AI\SD\input\小伙伴\A未分类\mmexport1734087306124.jpg" # 请确保这个路径存在
    ...
    # 创建 FaceFlip 实例并处理图片
    face_flip = FaceFlip()
    flipped_image, = face_flip.flip_images(
        image=img_tensor,
        auto_flip_face=True,
